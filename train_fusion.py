...
from metric import metric, do_valid_test, infer_test,do_valid_test_r
from model import get_fusion_model, get_model
from loss.cyclic_lr import CosineAnnealingLR_with_Restart
from process.data_helper_wmca import submission
from utils import *
...
def create_datasets(train_test):
    if train_test == 'SWP2C':
        ...
        train_dataset3 = FDDataset_wmca(mode='train', image_size=config.image_size,
                                        fold_index=config.train_fold_index)
        train_dataset = ConcatDataset([train_dataset1,train_dataset3,train_dataset2])
    ...
    for cycle_index in range(config.cycle_num):
        ...
        for epoch in range(0, config.cycle_inter):
            ...
            for input, truth in train_loader:
                ...
                # print("truth values (first 5):", truth[:5])
                ...
                # print("a_logit shape:", a_logit.shape)
                # print("a_logit values (first 5):", a_logit[:5])
                target=truth
                # print("target shape:", target.shape)
                
                

                truth = truth.view(logit.shape[0])  # 调整真值标签的形状
                ...
                # print("precision values (first 5):", precision[:5])
            ...
            log.write(model_name + ' Cycle %d: %0.4f %5.1f %6.1f | %0.6f  %0.6f  %0.3f  %0.3f %s  | %0.6f  %0.6f |%s \n' % (
                ...
                time_to_str((timer() - start), 'min')))  # 使用格式化字符串记录日志
...
def run_test(config, dir):
    # 根据配置构建模型名称
    model_name = config.model + '_' + config.image_mode + '_' + str(config.image_size)+ '_' + str(config.train_test)
    # 设置模型保存的根目录
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='MMDGFAS')
    ...
    parser.add_argument('--train_test',type=str,default='SCP2W',choices=['SCP2W','CWP2S','SCW2P','SWP2C','S2W'])
