...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 proj_drop=0., drop_path=0., init_values=None, act_layer=nn.GELU,
                 norm_layer=nn.LayerNorm, attn_head_dim=None, kernel_size=3, dilation=[1, 2, 3]):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = MultiDilatelocalAttention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=proj_drop, kernel_size=kernel_size, dilation=dilation)
    ...
    def forward(self, x):
        B, N, C = x.shape
        H, W = int(math.sqrt(N)), int(math.sqrt(N))  # Assumes square image patches
        x = x.reshape(B, H, W, C)  # Reshape for MultiDilatelocalAttention

        if self.gamma_1 is None:
            x = x + self.drop_path(self.attn(self.norm1(x)))
            x = x.reshape(B, N, C)  # Reshape back to original dimensions
...
class MultiModalViT(nn.Module):
    ...
    def __init__(self,
                 ...
                 is_multi_modal = True, kernel_size=3, dilation=[1, 2, 3]):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                init_values=init_values, kernel_size=3, dilation=[1, 2, 3])
