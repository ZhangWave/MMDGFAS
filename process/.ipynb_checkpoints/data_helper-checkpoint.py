...
DATA_ROOT =  r'F:/BaiduNetdiskDownload/CASIA_SURF/challenge/phase1'
TRN_IMGS_DIR = DATA_ROOT + '/train/Training/'
TST_IMGS_DIR = DATA_ROOT + '/valid/Val/'
RESIZE_SIZE = 112

def load_train_list():
    list = []
    f = open(DATA_ROOT + '/train/train_list.txt')
    lines = f.readlines()
...
def submission(probs, outname, mode='valid'):
    if mode == 'valid':
        f = open(DATA_ROOT + '/valid/val_public_list.txt')
    else:
        f = open(DATA_ROOT + '/test_public_list.txt')
