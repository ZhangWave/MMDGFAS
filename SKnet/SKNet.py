...
# class GDConv2(nn.Module):
#     def __init__(self, channels):
#         super(GDConv2, self).__init__()
...
#     def forward(self, x):
#         n, c, h, w = x.size()
#         # 在forward中动态创建权重，如果尚未定义或尺寸改变了
#         if self.weight is None or self.weight.size(2) != h or self.weight.size(3) != w:
#             # 初始化权重
#             self.weight = nn.Parameter(torch.randn(self.channels, 1, h, w)).to(x.device)
#         # 然后进行全局深度卷积操作...
#         # 执行全局深度卷积，对每个通道应用其学习到的权重
#         out = torch.zeros(n, c, 1, 1, device=x.device)
#         for i in range(c):
#             # 对第i个通道的特征图应用第i个通道的全局深度卷积核
#             out[:, i, 0, 0] = (x[:, i, :, :] * self.weight[i]).sum(dim=[1, 2])
...
class SKConv(nn.Module):
    def __init__(self,in_channels,out_channels,stride=1,M=2,r=16,L=32):
        ...
        # self.global_pool = GDConv(out_channels)
